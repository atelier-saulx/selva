...
export type Sliceable =
  | TypedArray
  | (GenericIndexable & { slice(start: number, end?: number): Indexable })
        ...
        hloop: for (let h = 0; h <= hmax; h++) {
          ...
          for (let k = kmin; k <= kmax; k += 2) {
            ...
            let ckm,
              ckp = b[Z + ((Zk + 1) % Z)]
            x = u =
              k === -h || ((ckm = b[Z + ((Zk - 1) % Z)]), k < h && ckm < ckp)
                ? ckp
                : ckm + 1
            y = v = u - k
            while (x < N && y < M && xs[xoffset - x] === ys[yoffset - y])
              x++, y++
            b[Z + (Zk % Z)] = x
            const z = delta - k
            if (
...
export function* applyPatch<T extends Sliceable>(
  xs: T,
  patch: Iterable<[number, number, T]>
): Generator<T> {
  let i = 0
  // Taking subarrays is cheaper than slicing for TypedArrays.
