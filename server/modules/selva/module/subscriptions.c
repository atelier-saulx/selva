...
static struct Selva_Subscription *find_sub(SelvaModify_Hierarchy *hierarchy, const Selva_SubscriptionId sub_id);
...
#if 0
    fprintf(stderr, "%s:%d: Destroying marker %p %d %.*s\n",
            __FILE__, __LINE__,
...
#if 0
    fprintf(stderr, "Adding marker %p %d\n", marker, marker->marker_id);
#endif
...
int SelvaSubscriptions_Refresh(RedisModuleCtx *ctx, struct SelvaModify_Hierarchy *hierarchy, const Selva_SubscriptionId sub_id) {
    struct Selva_Subscription *sub;
...
static void clear_node_sub(RedisModuleCtx *ctx, struct SelvaModify_Hierarchy *hierarchy, struct Selva_SubscriptionMarker *marker, const Selva_NodeId node_id) {
    ...
    if (dir &
        ...
        fprintf(stderr, "%s:%d: Clear sub marker %s:%d from node %.*s\n",
                __FILE__, __LINE__,
                Selva_SubscriptionId2str(str, marker->sub->sub_id),
                marker->marker_id,
                (int)SELVA_NODE_ID_SIZE, node_id);
        ...
        if (err && err != SELVA_HIERARCHY_ENOENT) {
            ...
            fprintf(stderr, "%s:%d: Failed to clear a subscription %s:%d: %s\n",
                    __FILE__, __LINE__,
                    Selva_SubscriptionId2str(str, marker->sub->sub_id),
                    marker->marker_id,
                    getSelvaErrorStr(err));
