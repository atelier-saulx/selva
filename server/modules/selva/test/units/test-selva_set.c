...
static char * set_intersection_longlong(void)
{
    ...
    SelvaSet_Add(&setA, 1ll);
    SelvaSet_Add(&setB, 1ll);
    SelvaSet_Add(&setC, 1ll);
    SelvaSet_Add(&setB, 2ll);
    SelvaSet_Add(&setC, 2ll);
    SelvaSet_Add(&setC, 3ll);
    SelvaSet_Add(&setA, 4ll);
    SelvaSet_Add(&setB, 4ll);
    SelvaSet_Add(&setC, 4ll);

    pu_assert_equal("intersection", SelvaSet_Intersection(&setD, &setA, &setB, &setC, NULL), 0);
    pu_assert_equal("resulting set size", SelvaSet_Size(&setD), 2);
    pu_assert_equal("resulting set has the right elems", SelvaSet_Has(&setD, 1ll), 1);
    pu_assert_equal("resulting set has the right elems", SelvaSet_Has(&setD, 4ll), 1);

    pu_assert_equal("source set size", SelvaSet_Size(&setA), 2);
    pu_assert_equal("source set size", SelvaSet_Size(&setB), 3);
    pu_assert_equal("source set size", SelvaSet_Size(&setC), 4);

    res = 0;
    SELVA_SET_LONGLONG_FOREACH(el, &setD) {
        res |= 1 << el->value_ll;
    }
    pu_assert_equal("foreach found all the elems", res, 0x12);
